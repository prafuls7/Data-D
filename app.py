...
class NSDLBondAnalyzer:
    ...
    def fetch_api_data(self, url):
        """Fetch data from NSDL API with error handling"""
        try:
    ...
    def perform_comparison(self, df, delay=0.5):
        ...
        for idx, row in df.iterrows():
            ...
            # Get coupon data separately
            coupon_data = self.get_instrument_data(isin)  # Same endpoint
...
def main():
    ...
    with st.sidebar:
        ...
        delay = st.slider("Delay between API calls (seconds)", 
                        0.1, 2.0, 0.5, 0.1,
                        help="To avoid rate limiting from NSDL servers")
        ...
        **Optional columns** for comparison:
        ...
        - Number of ISINs (≈1-2 seconds each)
        - Selected operations
        - Internet speed
        
        **Example**: 100 ISINs ≈ 2-3 minutes
        """)
    ...
    with col1:
        ...
        if uploaded_file:
            try:
                ...
                st.info(f"**Rows:** {len(df):,} | **Columns:** {len(df.columns)} | **Size:** {uploaded_file.size/1024:.1f} KB")
                ...
                if st.button("🚀 Start Processing", type="primary", use_container_width=True):
                    ...
                    if results:
                        ...
                        with col3:
                            st.metric("Total Time", f"~{len(df)*delay*len(results):.0f} sec")
